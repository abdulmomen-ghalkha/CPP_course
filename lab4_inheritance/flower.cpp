...
Flower::Flower(const Vector2& center)
{
	...
	stem = std::make_shared<Rectangle>(stem_center, STEM_COLOR, STEM_WIDTH, STEM_HEIGHT);

	pistil = std::make_shared<Circle>(Vector2(center.x(), center.y() - PISTIL_RADIUS/2), PISTIL_COLOR, PISTIL_RADIUS);

	const Vector2 leaf_center(stem_center.x() - STEM_WIDTH/2 + 1,
			stem_center.y() + STEM_HEIGHT/4);
	leaf = std::make_shared<Triangle>(LEAF_COLOR,
