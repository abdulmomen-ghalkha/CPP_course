...
void CommonWords::init_file_word_maps(const vector<string>& filenames) 
{
    ...
    for(size_t i = 0; i < filenames.size(); i++) {
        ...
        /* Your code goes here! */
        std::map< std::string, unsigned int > file_word_dict;
        for (const auto & word : words) {
            file_word_dict[word]++;
        }
        file_word_maps[i] = file_word_dict;
    }
}
...
vector< string > CommonWords::get_common_words(unsigned int n) const
{
    ...
    size_t num_words = common.size(); 
    for (const auto& pair : common) {
        if (pair.second >= num_files) {
            bool freq = true;
            for (const auto& word_map : file_word_maps) {
                auto word_it = word_map.find(pair.first);
